...
def compute_old_conv(input=None, old_params=None, scop=None, n_fil=None, **kwargs):
    ...
    for n in range(len(old_params)):
        ...
        old_c = activ(
            pgn_conv(input[n], 'old_' + scop + str(n), n_filters=n_fil, filter_size=3, stride=1,
...
def custom_cnn_pgn(scaled_images, old_params=None, **kwargs):
    ...
    for n in range(len(old_params)):
        param = old_params[n]
        old_c = activ(
            pgn_conv(scaled_images, 'old_c1' + str(n), n_filters=32, filter_size=3, stride=1,
        ...
        with tf.variable_scope('model', reuse=reuse):
            ...
            if old_params:
                ...
                self._proba_distribution, self._policy, self.q_value = \
                    self.pgn_pdtype.proba_distribution_from_latent(pi_latent, vf_latent, init_scale=0.01,
